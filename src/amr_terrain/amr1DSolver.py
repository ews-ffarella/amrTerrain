...
class amr1dSolver:
    ...
    def run_simulation(
        ...
        self.compute_zi()
        target = open(self.writePath, "w")  # noqa: SIM115
        for i in range(0, len(self.z)):
            target.write(
                "%g %g %g %g %g %g %g %g \n"
                % (
                    self.z[i] - self.terrain_height,
                    self.ux[i],
                    self.uy[i],
                    0.0,
                    self.temperature[i],
        ...
        for i in range(0, self.nz):
            ...
            if self.z[i] <= self.inversion_height:
                ...
                if self.z[i] <= self.z[self.lower]:
                    ...
                    self.temperature[i] = self.temperature[i - 1] + self.lapse_rate * max(
                        self.z[i] - self.z[i - 1], 0.0
                    )
            phiM = 1 + 5 * self.z[i] / self.mode_value if self.heat_flux_mode == 4 and self.mode_value > 0 else 1
    ...
    def compute_similarity(self):  # noqa: C901
        ...
        if self.heat_flux_mode == 1:
            ...
            while iter <= 25 and error > 1e-5:
                ...
                self.temperature[self.lower] = (
                    self.Qh * (np.log(self.z[1] / self.z0) - psi_h) / (self.ustar * 0.41)
                    + self.temperature[self.lower + 1]
                )
                if abs(self.Qh) > 1e-5:
                ...
                if zeta >= 0:
                    ...
                    psi_m = np.log(0.5 * (1 + x**2) * 0.25 * (1 + x) ** 2) - 2.0 * np.arctan(x) + 0.5 * np.pi
        ...
        # Surface temperature specified. Compute sensible heat flux and MOL
            ...
            if self.temperature[self.lower] == self.t_ref:
                self.Qh = (
                    (self.temperature[self.lower] - self.temperature[self.lower + 1])
                    * (self.ustar * 0.41)
                    / (np.log(self.z[1] / self.z0) - psi_h)
                )
                self.mo_length = -1e30
            else:
                while iter <= 25 and error > 1e-5:
        ...
        # Used for Operational Runs
            ...
            self.temperature[self.lower] = self.temperature[self.lower + 1] - self.thetastar / 0.41 * (
                np.log(self.z[1] / self.z0) - psi_h
            )
    ...
    def update_windspeed_x(self, i, dt):
        ...
        dudz = 1 / self.dz * (self.ux[i + 1] - self.ux[i]) if i == 1 else 1 / self.dz * (self.ux[i + 1] - self.ux[i])
    ...
    def update_windspeed_y(self, i, dt):
        ...
        dvdz = 1 / self.dz * (self.uy[i + 1] - self.uy[i]) if i == 1 else 1 / self.dz * (self.uy[i + 1] - self.uy[i])
    ...
    def update_temperature(self, i, dt):
        term1 = (
            self.nut[i]
            / self.sigmaT[i]
            * (self.temperature[i + 1] - 2 * self.temperature[i] + self.temperature[i - 1])
            / self.dz**2
        )
        term2 = (
            1
            / self.dz
            * (self.nut[i] / self.sigmaT[i] - self.nut[i - 1] / self.sigmaT[i - 1])
            * 1
            / self.dz
    ...
    def update_turbulence(self, i, dt):
        ...
        production = (
            self.nut[i] * (1 / self.dz) ** 2 * ((self.ux[i] - self.ux[i - 1]) ** 2 + (self.uy[i] - self.uy[i - 1]) ** 2)
        )
        # Original RANS Model
    ...
    def compute_zi(self):
        ...
        Rib = (
            ...
            * self.z
            / self.temperature[self.lower]
            * (self.temperature - self.temperature[self.lower])
            / (M + 1e-5)
        )
        Ric = 0.5
        zi = max(self.z)
    ...
    def print_mean_results(self):
        ...
        for i in range(0, self.nz):
            if zeta[i] > 0:
                ...
                psi_m[i] = np.log(0.5 * (1 + x**2) * 0.25 * (1 + x) ** 2) - 2.0 * np.arctan(x) + 0.5 * np.pi
        plt.semilogx(self.z, np.sqrt(self.umean**2 + self.vmean**2))
        plt.semilogx(
            self.z,
            self.ustar / 0.41 * (np.log((self.z + self.z0) / self.z0) - psi_m),
            "ro",
        )
        plt.xlim(5, 200)
