...
class SRTM(Terrain):
    ...
    data_products = {  # noqa: RUF012
        "SRTM1": 30.0,
        "SRTM3": 90.0,
    }

    def __init__(self, latlon_bounds, fpath="terrain.tif", product="SRTM3", margin=0.05):
        ...
        assert product in self.data_products, "product should be one of " + str(list(self.data_products.keys()))
...
class USGS(Terrain):
    ...
    def _get_bounds_from_metadata(self):
        ...
        if self.metadata.tag == "metadata":
            ...
            bounds = [float(bounding.find(bcdir + "bc").text) for bcdir in ["west", "south", "east", "north"]]
        else:
            # ISO XML
            extent = self.metadata.find("gmd:identificationInfo/gmd:MD_DataIdentification/gmd:extent", ISO_namespace)
            bbox = extent.find("gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox", ISO_namespace)
            bounds = [
...
def combine_raster_data(filelist, dtype=Terrain, latlon_bounds=None, output="output.tif"):
    ...
    terraindata = [dtype(bounds, fpath) for bounds, fpath in zip(latlon_bounds, filelist, strict=False)]

    # merge rasters
    from rasterio.merge import merge

    merged, out_transform = merge([rasterio.open(data.tiffdata) for data in terraindata])
...
def calcTRI(hgt, window=None, footprint=None):
    ...
    if isinstance(hgt, (xr.Dataset, xr.DataArray, xr.Variable)):
        hgt = hgt.data
    assert len(np.shape(hgt)) == 2, f"hgt must be 2-dimensional. Currently has {len(np.shape(hgt))} dimensions"
...
def calcSx(xx, yy, zagl, A, dmax, method="linear", verbose=False):
    ...
    for i, xi in enumerate(xrot):
        if verbose:
            print(f"Computing Sx... {100 * (i + 1) / len(xrot):.1f}%  ", end="\r")
        for j, yi in enumerate(yrot):
            ...
            Sxrot[i, j] = np.nanmax(
                np.rad2deg(np.arctan((elev[:-1] - elevi) / (((xsel[:-1] - xi) ** 2 + (ysel[:-1] - yi) ** 2) ** 0.5)))
            )
...
def extract_elevation_from_stl(stlpath, x, y, interp_method="cubic"):
    ...
    if len(x) == 1:
        return np.array(list(zip(x, y, elev, strict=False))[0])
    else:
        return np.array(list(zip(x, y, elev, strict=False)))
...
def readSTL(stlpath, stlres=None, method="cubic"):
    ...
    if stlres is None:
        print(
            f"Using {apparentres} m resolution obtained from the STL. This resolution may not be entirely "
            "accurate. If not, please provide a proper value using the `stlres` parameter."
        )
    elif stlres != apparentres:
        print(
            f"The {stlres} m resolution provided does not match"
            " what the function thinks the resolution is, {apparentres} "
